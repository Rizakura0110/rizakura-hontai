...
## Phase 46: Daymark習慣削除の本番反映

2026-09-24、所有者承認後に基盤`e45dcff`（Daymark `c5afa87`）を既存`rizakura-hontai` Workerへ反映しました。versionは`5c6f6360-6937-4c27-9cf2-28d2b0889289`（100%）。DB migration・新規resource・料金・Access変更、Toki/metadata-fetcherの再deployはありません。反映前後の認証・binding/schema・公開範囲・health検査は成功し、本番ブラウザで削除確認とキャンセルを確認しています。実データの削除は実施していません。

Daymarkの「習慣管理→編集→習慣を削除」で対象名と消去範囲を確認し、「習慣と全記録を削除」で完全削除します。設定履歴・記録も消え、日/週/月の履歴と集計から除外されます。取り消し・ゴミ箱はありません。履歴を残すなら休止・アーカイブを使ってください。重要な習慣を消す前にはDaymark JSONを保存してください。保存済みの古いbackupは変更されず、明示的に復元すると削除済み習慣が再登録される場合があります。

所有者のPC/iPhone操作と本番の削除確定は未確認です。テストのために既存の習慣を勝手に削除しません。アプリ更新のrollbackは削除済みDB行を復元しないため、誤削除の復旧はJSONまたは承認付きTime Travelとして別に検討します。
