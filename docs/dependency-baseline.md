...
- Phase 33は進行中。所有者承認済みpublic `Rizakura0110/tech-inbox`を作成し、`modules/tech-inbox`の固定commit submoduleと`workspace:0.0.0`で取り込む構成を検証している。package名と`private: true`を維持し、npm公開やGit URLのpackage dependencyは使わない。Daymarkと同様、製品自身のworkspace・lockfile・CI・auditと、基盤側の結合lockfile・全gateを分ける。
...
- Phase 32後続CIのclient bundle上限超過を受け、Phase 33で独立install間の共有runtimeをdedupeする。基盤rootのdevDependenciesへ既存versionのReact 19.2.8、React DOM 19.2.8、React Router 8.3.0、Zod 4.4.3、Drizzle ORM 0.45.2を明示し、appにも既存Zod 4.4.3を明示する。ViteとVitestの共有解決先を安定させるためのimporter変更であり、第三者versionの更新や供給網policyの緩和ではない。Testing Libraryも基盤の既存固定依存へ解決を統一する。
- Tech Inbox単体260 testsと独立audit、基盤598 testsは成功した。全品質gate・clean checkout・CIの最終結果は[Progress](progress.md)へ記録し、それらが揃うまではPhase 33を完了扱いにしない。
