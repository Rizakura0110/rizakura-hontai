...
- 原因は1日1440pxに対して短い記録の最低高さが0.75%（約11px）しかなく、時刻の下にある内容が隠れることだった。1時間120px（従来の2倍）、表示枠の最低高さ44pxへ変更し、内容を先頭に置き縮小されない1行表示とした。長文は横方向に省略し、全文・正確な時刻は記録を開いて確認できる。
- 見た目の高さと実時間を分け、拡大後の枠同士が重なる場合の横並び配置・日末での表示枠補正も新しい高さと一致させる。短い記録の見た目の長さは実時間に比例しない旨を注記した。保存時刻・API・DB schema・認証・依存は不変。
- Tokiの`pnpm check`が成功。12 files/194 tests、coverageはstatements 90.16%・branches 89.04%・functions 97.77%・lines 93.86%。format/lint・生成型・TypeScript・local D1・dry-run build・E2E 7 tests・audit（脆弱性0件）を通過した。
...
- 本番用の非追跡`stage`設定を生成・dry-runし、Toki Workerを`workers_dev:false`、preview無効、設定上route/custom domain指定なしで非公開配置した。実際のWorker設定は新Toki D1の`DB` bindingと`ASSETS` bindingのみで、匿名でTokiの`workers.dev` URLは404だった。
