...
import { allocateTagColorHue, MAX_TAGS, normalizeTagName } from "../core/tag";
...
export class TagService {
  ...
  async create(request: CreateTagRequest): Promise<CreateTagResponse> {
    ...
    if (!normalized.ok) throw new TechInboxError("VALIDATION_ERROR", "タグ名が無効です。");
    ...
    for (let attempt = 0; attempt < MAX_COLOR_ALLOCATION_ATTEMPTS; attempt += 1) {
      ...
      if (currentTags.length >= MAX_TAGS) {
        throw new TechInboxError("TAG_CONFLICT", "タグ数の上限に達しています。");
      }
      const colorHue = allocateTagColorHue(currentTags.map((tag) => tag.colorHue));
      if (colorHue === null) {
        throw new TechInboxError("TAG_CONFLICT", "タグ色を割り当てられませんでした。");
      }
    ...
    throw new TechInboxError("TAG_CONFLICT", "タグ色の割り当てが競合しました。");
  }
  ...
  async update(id: string, request: UpdateTagRequest): Promise<TagResponse> {
    ...
    if (result.outcome === "notFound") {
      throw new TechInboxError("NOT_FOUND", "タグが見つかりません。");
    }
    if (result.outcome === "nameConflict") {
      throw new TechInboxError("TAG_CONFLICT", "同じ名前のタグが既に存在します。");
    }
