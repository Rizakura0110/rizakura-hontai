...
describe("parseHtmlMetadata", () => {
  it("collects the first matching HTML values and delegates to safe selection", async () => {
    ...
    class FakeHtmlRewriter {
      readonly handlers = new Map<string, MetadataHtmlHandlers>();

      on(selector: string, handler: MetadataHtmlHandlers) {
        ...
        return {
          async text() {
            for (const [selector, entries] of values) {
              ...
              if (selector === "title") {
                for (const value of entries) handler?.text?.({ text: value });
                continue;
              }
