import { CONTRACT_LIMITS, type FetchedMetadata } from "../contracts";

// Only the operations required by metadata extraction cross the runtime boundary.
// The Cloudflare entrypoint provides HTMLRewriter; the product owns no CF globals.
export type MetadataHtmlElement = {
  getAttribute(name: string): string | null;
};

export type MetadataHtmlHandlers = {
  readonly element?: (element: MetadataHtmlElement) => void;
  readonly text?: (text: { readonly text: string }) => void;
};

export type MetadataHtmlRewriter = {
  on(selector: string, handlers: MetadataHtmlHandlers): MetadataHtmlRewriter;
...
export async function parseHtmlMetadata(
  response: Response,
  finalUrl: string,
  createRewriter: () => MetadataHtmlRewriter,
  ...
  const readContent = (assign: (content: string) => void) => ({
    element(element: MetadataHtmlElement) {
